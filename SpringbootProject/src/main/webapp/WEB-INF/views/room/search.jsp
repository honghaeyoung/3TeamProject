	...
	<main>
		...
		<div class="container">
			...
			<p>${count}개가 검색되었습니다</p>
				<c:forEach items="${sList}" var="rooms">
				...
				<div class="btn-toolbar d-md-flex justify-content-end" id="page">
					...
					<c:if test="${begin>pageNum}">
						<a href="list/search?p=${begin-1}" class="btn btn-outline-secondary">이전</a>
					</c:if>
					<c:forEach begin="${begin}" end="${end}" var="i">
						<a href="list/search?p=${i}" class="btn btn-outline-secondary">${i}</a>
					</c:forEach>
					<c:if test="${end<totalPages}">
						<a href="list/search?p=${end+1}" class="btn btn-outline-secondary">다음</a>
					</c:if>
			...
			<div class="d-flex justify-content-center">
				<form action="/room/list/search" method="get">
					<div class="input-group mb-3">
						<select name="searchn" class="form-select form-select-sm" id="inputGroupSelect01" style="width:90px;">
